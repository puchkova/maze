# Maze
The project is the NodeJS server-side application built using Express framework. For automated testing the Jest framework and the Supertest library were used. It is possible to run the application in a Docker container.

The point of this application is to get the minimum number of steps needed to go through the maze. The maze is a rectangular matrix with maximum size of 30x30. Every cell can be empty (.) or be a wall (#). It is not allowed to go through the walls. The starting point is the upper left point and the end is the lower right point.   
...
- `labyrinth.js` - The file contains 3 functions: 
	 - `isMazeValid` - This function checks if the matrix is rectangular, contains only allowed values, is not larger than allowed size and not empty. 
...
- `docker-compose.yml` - This file contains the configuration to set up the docker environment.
